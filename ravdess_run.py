...
from wav2vec import Wav2Vec2ForCTCnCLS
        ...
        for k, v in inputs.items():
            ...
            if k == 'labels': # labels are list of tensor, not tensor, special handle here
                for i in range(len(inputs[k])):
                    ...
                    print(inputs[k])
                    print(inputs[k][i])
                    inputs[k][i] = inputs[k][i].to(kwargs["device"])
...
def main():
    ...
    if data_args.dataset_name == 'emotion':
        train_dataset = datasets.load_dataset('csv', data_files='train_data.csv', cache_dir=model_args.cache_dir)['train']
        val_dataset = datasets.load_dataset('csv', data_files='val_data.csv', cache_dir=model_args.cache_dir)['train']
        cls_label_map = {"neutral": 0, "calm": 1, "happy": 2, "sad": 3, "angry": 4, "fearful": 5, "disgust": 6, "surprised": 7}
    ...
    def prepare_dataset(batch):
        ...
        cls_labels = list(map(lambda e: cls_label_map[e], batch["label"]))
    ...
    if training_args.do_predict:
        ...
        with open(data_args.output_file, 'w') as f:
            for i in range(len(pred_ids)):
                f.write(val_dataset[i]['filepath'].split("/")[-1] + " " + str(len(val_dataset[i]['input_values'])/16000) + " ")
                pred = pred_ids[i]
                f.write(str(pred)+' ')
                for j in range(4):
